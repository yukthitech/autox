...
/**
 * Used to handle Ignore error request.
 * @author akiran
 */
public class IgnoreErrorHandler extends AbstractServerDataHandler<ClientMssgIgnoreError>
{
	private static Logger logger = LogManager.getLogger(IgnoreErrorHandler.class);
	
	public IgnoreErrorHandler()
	{
		super(ClientMssgIgnoreError.class);
	}
	
	@Override
	public void processData(ClientMssgIgnoreError data)
	{
		logger.debug("Executing ignore error request: {}", data);
		...
		boolean res = livePoint.requestIgnoreError(data);
